...
public class ClassicCapPageChrome extends AbstractComponent {
	WebDriver driver;

	public ClassicCapPageChrome(WebDriver driver) {
		super(driver);
		this.driver = driver;
	...
	WebElement opt_ColourChoose;
	
	@FindBy(xpath = "//ul[contains(@class,'select2')]/li/div/span")
	List<WebElement> opt_Colours;
	...
	public void titleVal(String actualPageTitle, String expectedPageTitle) {
		...
		if (actualPageTitle.equalsIgnoreCase(expectedPageTitle)) {
			System.out.println("Classic Cap Page, Actual Title Matches with the Expected Title");
		} else {
			System.out.println("Classic Cap Page, Actual Title doesn't Matches with the Expected Title");
		}
	}
	...
	public void classicCapSizeSelect() throws Exception {
	...
	public void colourChoose(String colourCC) throws Exception {
		assertTrueElementDisplayed(opt_ColourChoose);
		Thread.sleep(2000);
		for (WebElement colour : opt_Colours) {

			if (colour.getText().equalsIgnoreCase(colourCC)) {
				waitForElementToBeClickable(colour);
				colour.click();
				break;
			}
	...
	public void classicCap_Submit() throws Exception {
